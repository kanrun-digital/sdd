...
This skill projects the upstream artifacts into one **interface contract**. By default that contract is an HTTP/OpenAPI contract. The skill is **interface-kind aware**. The kind comes from the surface(s) `design` declared in `sad.md` frontmatter `target_surfaces`. Read it here — do not re-derive it (→ [`../_shared/surfaces.md`](../_shared/surfaces.md)). For a non-HTTP project the skill produces the matching contract form (or steps aside):
...
Whatever the form, the contract is **derived from `data-model.md` — or, on a legal no-schema-change skip, the existing schema — plus the sad.md §6 sequences + the spec's AC. It is never typed by hand.** Generation that diverges from the model or the sequences is the bug this skill exists to catch. The rest of this file details the HTTP path (the common case). The same derive → drift-check → reconcile loop applies to the other forms. Each form uses its matching artifact.

This skill keeps only its own machinery. Question phrasing is **shared** → [`../_shared/ask-style.md`](../_shared/ask-style.md). Depth follows the **size matrix**: events doc only when async, one resource vs full surface → [`../_shared/size-matrix.md`](../_shared/size-matrix.md). The drift-resolution dialog reuses the shared 4-state actions. Keep it short. Point the machinery to `_shared`.
...
- **Gate (conditional — hard-refuse only when a schema change exists):** `docs/features/<slug>/data-model.md`. When present, it is the source of typed fields and constraints. When absent, evaluate `data-model`'s N/A condition yourself (no schema change — [size-matrix fast lane](../_shared/size-matrix.md)): sad.md §5 declares no new building blocks/entities, no staged `docs/features/<slug>/migrations/`, and the spec introduces no new entity → **proceed**. Derive types/constraints from the **existing schema** (the live `migrations/` DDL + `architecture-map.md` §Migrations/§Conventions). Say so loudly in the handoff. Absent **and** a schema change exists → STOP and point: «run `data-model <slug>` first — the contract is derived from its entities».
...
4. **Derive error responses from the sequences.** For each endpoint covered by a §6 flow, turn every `alt … else … end` branch into a `responses` entry. The error body is the unified envelope **`{code, message, details?}`**. `code` follows the **neutral** convention `module.error_name` (snake_case, e.g. `lesson.not_owned`, `lesson.invalid_state`) — a naming rule, not a language artifact. Map the status per the **business-error → HTTP status matrix** in [`./references/drift-check.md`](./references/drift-check.md) Defaults section (conflict→409, validation→422, malformed→400, not-found→404, hide-existence→404-never-403, unauth→401, no-perm→403, rate-limited→429, server→5xx) — not just «4xx/5xx by class». If `spec.md §6.1` names an abuse-case signal (spam-create, scraping, brute-force), also model the **`429` + `Retry-After`** rate-limit response per the drift-check rate-limiting default. This closes the spec's usual blind spot. §5 lists the happy path + a couple of errors. The sequences enumerate the authorization and concurrent-state branches the spec omits.
...
7. **Inline DRIFT CHECK (bidirectional) + write the report.** Compare the generated contract against the read artifacts and write `docs/features/<slug>/contracts/api-sync-report.md` — see [`./references/drift-check.md`](./references/drift-check.md). It has a field-origins table (one row per `operation.field`: `path | origin | confidence`) and a checklist. The check runs **both directions**:
   ...
   - **back-feed (coverage cross-check)**: every `spec.md` §5 AC maps to ≥1 operation/response. Every operation maps to a §4 user story + ≥1 AC. Every `sad.md` §6 `alt`-branch has a response. Any error/authorization response the contract needs but no §6 flow shows is a **sequence gap**. A gap here is not an api bug — it is a hole upstream. Report it and resolve it as **Save-as-OQ with the upstream stage as owner**. The OQ row names the producing stage as owner (`specify` for a missing AC, `sequences` for a missing branch) with due «before the contract is finalized». The source then gets fixed through the standard 4-state machine, not a fifth action.
   A **core** finding failing (or ≥3 flags total) pauses the run — resolve each via the shared 4-state actions ([`../_shared/ask-style.md`](../_shared/ask-style.md)): Accept / Fix (the contract) / Save-as-OQ / Drop. A fix that belongs upstream (the spec's AC, the sequence) is the **Save-as-OQ variant with the upstream stage as owner** (see step 7's back-feed) — never a fifth action. Never silently edit the sources. Report the mismatch. Let the human pick the right artifact (the contract, the spec's AC, or the sequence).
8. **Lint + write + commit.** Suggest `spectral lint contracts/openapi.yaml` (add it to the project's check target if not yet wired). On a clean check, write the files. Propose commit `api: <slug> contract`. Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) — *What I did* + *Review* (`contracts/openapi.yaml`, `api-sync-report.md`, + `events.md` if async) + *Run next* (`/clear`, then `/sdd:tasks <slug>`).

### Reconcile mode

`/sdd:api <slug> --reconcile`. Re-derive after an upstream artifact changed (typically `data-model.md` arrived or got tighter after a thinner first pass). Re-read the inputs. Tighten loose types where the model now has a constraint. Refresh the field-origins confidence column. The load-bearing part: report any field that **had** an inferred origin but **now disagrees** with the model. That disagreement is real drift, not stale incompleteness. Never bump `info.version` silently. The user bumps it with a CHANGELOG line.
...
- The step-7 bidirectional drift check + `api-sync-report.md` are this skill's **structural self-check** ([`../_shared/self-check.md`](../_shared/self-check.md)). Report its result in the handoff.
...
- **Inventing a field** with no origin in any input, or **silently dropping** one that left `data-model.md` (keep it with a `# stale` note and report it — the human decides).
