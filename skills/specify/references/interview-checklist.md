...
The depth dial ([`../../_shared/interview-depth.md`](../../_shared/interview-depth.md)) and the shared [`../../_shared/ask-style.md`](../../_shared/ask-style.md) option-writing contract govern the step-2 deep-dive. **This file is the content contract** — the dimensions the interview must surface so the spec template has real input to draft from. The depth dial tunes *volume*, never *coverage intent*. At `easy` the skill infers the un-asked dimensions. It records each as a §10 Assumptions row. At `hard` it walks every dimension with a trade-off foregrounded.
...
- **No dimension is silently skipped.** At every depth, every dimension is either *asked* (medium/hard), *inferred + recorded as a §10 Assumption* (easy), or *marked N/A with a reason*. Example: dimension 10 on a greenfield feature → `<!-- N/A: greenfield, no migration -->` in §3.
- **Batch discipline.** Questions go in batches of 2–3 per [`../../_shared/ask-style.md`](../../_shared/ask-style.md). One `AskUserQuestion` per batch. Do not fire 10 at once.
- **The dimensions map 1:1 onto spec sections** — a gap in a dimension shows up as a stub in its target section. The critic (F5/F7/F9) then flags it.
...
If any of the three is still incoherent after the deep-dive batches, fire **one more mini-batch** of 2–3 questions. Target the unclear answer. Do not proceed to the ideation suite on a fuzzy problem framing. (At `easy` depth, infer the missing answer, record it as a §10 Assumption, and let the user veto on the ledger. The gate is softer, not absent.)
