...
- **Acceptance criterion** (§5) — the 4-state machine **plus a 5th option «Add another AC»** (user dictates a new AC. The skill drafts it in business form and runs a one-question mini-batch on it). Dropping / OQ-migrating the **last AC of a retained §4 user story** fires the use-case floor below (regenerate an AC for that US).
...
2. **Use-case floor (§4 US → ≥1 §5 AC).** **Every *retained* §4 user story still has ≥1 acceptance criterion.** If a Drop / OQ-migration leaves a retained US with no AC, regenerate (or use the «Add another AC» option) an AC for that US. Run a one-question mini-batch. A user story with no AC is incomplete. It silently breaks the downstream `sequences` use-case coverage and `review`'s end-to-end trace. (Dropping the **whole** US is a legitimate de-scope. It does not fire this floor. Only a retained US losing its last AC does.) This also applies at **draft time**: if the initial §5 draft gave some §4 US no AC, add one before the walk begins.

## Open-Questions table

`save_as_oq` rows land in **§8 Open questions** as a checkbox line: `- [ ] <headline>? — owner: <…>, due: <…>`. Owner + due are mandatory. A missing one downgrades the row to Drop.
