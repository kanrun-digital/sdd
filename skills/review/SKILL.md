...
- (Optional, project-level override) `docs/.skill-context/sdd-review/SKILL.md`. If it exists, read it and treat its rules as project-level overrides. On conflict they win. Apply them to all outputs → [`../_shared/skill-context.md`](../_shared/skill-context.md). If absent, do nothing (defaults apply).
...
2. **Dispatch the independent reviewer.** Run the [`reviewer`](../../agents/reviewer.md) agent — `subagent_type: "sdd:reviewer"` (read-only, **clean context** — it re-reads spec/contracts itself, no paraphrase. **Model per `judgment_model`, effort `xhigh` on L/XL** via `CLAUDE_CODE_EFFORT_LEVEL`, per [`../_shared/agent-roster.md`](../_shared/agent-roster.md)) — over the diff along the dimensions in [`./references/review-dimensions.md`](./references/review-dimensions.md). **Stage 1** — check every claimed AC is genuinely satisfied. Trace the whole §4 user-story set + §5 AC set end-to-end (spec → sequences §6 → data-model → api → tasks → implement). Every §4 user story must have ≥1 AC and a §6 flow. Every §5 AC must reach code+test. The trace spans every surface declared in `sad.md` `target_surfaces` (a UI AC traces to a component / e2e-through-UI test, not only a backend one). Flag any user story or AC that dropped out anywhere in the chain — not only the ACs the diff claims via its `SDD-AC` trailers. Stage 1 also checks **ADR conformance** (every Accepted ADR cited to a realizing `file:line` or N/A). **Stage 2** — conventions, error/edge handling, **performance/hot-path, security (16-item OWASP-derived checklist), accessibility (conditional — runs iff `target_surfaces` declares a UI surface, then stage-1 blocking), observability (declared-metric-emission check), test adequacy (incl. the litmus test), complexity/over-engineering (5-tag ponytail scan)**, boundary violations. Every finding carries a severity (`critical`/`high`/`medium`/`low`). A `critical` stage-2 finding promotes to stage-1 (blocking). Apply the 5-flag finding-qualification gate — drop speculative/style-nit noise. For a large diff, dispatch one reviewer per dimension and merge.
3. **Collect cited findings.** Each finding cites `file:line` + the AC/contract it touches. Drop uncited findings (per the critic discipline). A clean review returns `REVIEW_CLEAN`. If the reviewer ran asynchronously and came back as an idle/completion signal with no report, **fetch the full report through the host's messaging channel** — never accept a verdict without its text (→ [`../_shared/agent-roster.md`](../_shared/agent-roster.md), shared-contract point 2).
4. **Resolve each finding with the user** via `AskUserQuestion`: **Fix now** (return the actionable finding to `implement`/the author as a follow-up task — re-enter the TDD loop for it) / **Defer** (record in spec §8 Open questions with owner + due) / **Not an issue** (the reviewer misread — record why). Never ship an unresolved stage-1 (AC) finding.
...
- **Shipping with an open AC finding.** A stage-1 gap means the feature doesn't do what the spec says — fix or explicitly de-scope (spec change), never approve it silently.
...
- **Deferring a stage-1 finding without owner+due.** A stage-1 (AC/contract/ADR-conformance) finding is blocking — the only legitimate exits are Fix-now, explicit de-scope (spec change with owner), or Defer with owner+due captured in §8. A bare "Defer" that ships is a silent regression.
- **Treating a11y as non-blocking for a UI-surface feature.** When `sad.md` `target_surfaces` declares a UI surface, the accessibility dimension is **stage-1** — compliance risk (WCAG, legal exposure) outranks the usual "quality is non-blocking" posture. Demoting it to stage-2 to approve silently is an anti-pattern.
- **Noise-review.** Flagging speculative concerns, pre-existing problems, or style nits. Use the 5-flag finding-qualification gate (review-dimensions.md) — better `REVIEW_CLEAN` than a report the author will dismiss.
