...
- **Treating a11y as non-blocking for a UI-surface feature.** When `sad.md` `target_surfaces` declares a UI surface, the accessibility dimension is **stage-1** — compliance risk (WCAG, legal exposure) outranks the usual "quality is non-blocking" posture. Demoting it to stage-2 to wave through is an anti-pattern.
