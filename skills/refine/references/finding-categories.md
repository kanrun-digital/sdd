...
The groups are processed in a fixed order. The report is then scannable. The `+check` recomputation of dependencies (phase b) then has a deterministic post-state:
...
`dependencies` is **always** computed last. The other four groups are finalized first (and `+check` filtering runs when the flag is set). The DAG is then recomputed against the post-refinement task set. It is never sent to the validator.
...
1. **Behavioral impact** — one or two sentences: what breaks, becomes harder, or wastes effort if the plan stays as-is. It is the *why this matters*, tied to a concrete behavior (a missing capability, a vague task that will be misimplemented, a duplicate that burns cycles).
...
A gap in the plan: a piece of work the feature requires but no task covers. This is the most common source of rework. The `tasks` first pass routinely misses these.
...
A task that should not be in the plan. It is distinct from `out_of_scope` (group 4). A `removals` task is *dead weight* — it duplicates work or targets code that exists. An `out_of_scope` task is *useful but unrelated*.
...
**The drop action is identical to `removals`** — the task is removed from the plan. The difference is **report-only**. An `out_of_scope` task appears in the «💡 Out of scope» section. The user then sees a useful idea before it is dropped. It is not lumped into «🗑️ Removals» as dead weight.
...
- **Unnecessary serialization (improvement, not its own finding unless critical-path)**: two tasks with disjoint `files_hint` are serial when they could parallelize. Flag it as an `improvements` finding («T5 and T6 can parallelize — disjoint files_hint»). Exception: the chain is on the critical path. Then it is a standalone dependency finding. A deep serial chain of S tasks is a hidden multi-sprint blocker. This is the J2 critical-path-depth warning from `tasks`.
