...
Draws the **runtime view** of an already-designed feature. For each critical flow it produces a Mermaid `sequenceDiagram` block — generic participants, happy path plus the error branches the spec demands — and writes it into `docs/features/<slug>/sad.md §6`. One flow at a time, the user confirms each. The diagrams are the bridge between the static design (§5 building blocks) and the data layer: every persist/read step you draw becomes a hint for the indexes `data-model` will need.
...
- **Strongly expected:** `docs/features/<slug>/spec.md` — §4 user stories tell you *which* flows exist. §5 acceptance criteria are the **coverage floor** — every AC must be shown by a flow, a branch, or an explicit non-runtime N/A (the step-7 coverage check). Present by this stage in the normal pipeline. If genuinely absent, fall back to §6/§5 of `sad.md` for the flow list and note that AC-coverage can't be checked.
...
- (Optional, project-level override) `docs/.skill-context/sdd-sequences/SKILL.md` — if it exists, read it and treat its rules as project-level overrides (conflict → they win and apply to all outputs) → [`../_shared/skill-context.md`](../_shared/skill-context.md). Absent → no-op (defaults apply).
...
4. **Sync vs async.** If the spec describes a webhook, scheduled job, queued/event-driven step, or any third-party callback → async: add an idempotency-key check as the handler's first step, a retry note (`Note over <service>,<external-system>: retry N times with backoff`), and a dead-letter branch in an `alt` after N failures. Otherwise → sync (request → response).
   - **Three conditional flow types (W5 — the distributed-system gaps the async template alone misses).** After classifying sync/async, check whether the flow needs a **compensation / timeout-degrade / concurrency-conflict** flow (templates in [`./templates/sequence.md`](./templates/sequence.md)):
     - **Compensation (saga rollback)** — draw iff the flow chains **≥2 async actors** where a later step can fail after an earlier step already committed. The dead-letter branch only quarantines the failing step. It does NOT undo the steps that succeeded. N/A is valid with a reason («each step independently idempotent + DLQ-only is an accepted policy — ADR-NNNN»).
     - **Timeout / degrade** — draw iff the flow includes an **`<external-system>` participant** and the spec's success criteria cannot tolerate indefinite blocking. Draw the timeout → fallback (cached / default / degraded) branch. N/A valid («fire-and-forget / has its own async retry»).
     - **Concurrency / optimistic-lock conflict** — draw iff **≥2 writers can race on the same entity** (two requests mutating the same record). Draw the version-mismatch → reject + retry branch. N/A valid («single-writer guarantee elsewhere — e.g. a queue serializes writes»).
     - These are **gated, not universal** — draw when the signal is present, N/A with reason otherwise. An async saga with no compensation flow and no N/A is a gap.
5. **Draft each flow** from [`./templates/sequence.md`](./templates/sequence.md): a precondition note, the happy-path messages, an `alt`/`else` for the error branches the spec's acceptance criteria require, and a postcondition note. Mark every write as a generic persist note — `Note over <service>,<data-store>: persists <entity>` — so `data-model` sees what to index. Keep messages verb-first and free of HTTP verbs / status numbers / SQL. **For the conditional flow types (compensation / timeout / concurrency), if you drew one, its persist notes + emitted-metric notes also feed `data-model` indexes and `§8` observability.**
...
7. **Use-case + AC → flow coverage check (before finalizing).** Two passes, reported as one coverage table:
...
- The step-7 use-case + AC coverage check + the step-8 mermaid re-validation are this skill's **structural self-check** ([`../_shared/self-check.md`](../_shared/self-check.md)). Report its result in the handoff.
