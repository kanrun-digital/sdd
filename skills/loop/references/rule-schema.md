...
PREPARE expands every rule in `run.json.criteria.rules` to full schema form. Persistence holds no shorthand. PREPARE expands any template shorthand before EVALUATE runs:
...
- `weight` — used in score calculation. `fail` rules carry the most (default 2). A single fail therefore drags the score more than a warn.
...
- `warn`/`info` never force `passed = false`. CRITIQUE surfaces them. They do not block.

## Default rule-sets by artifact type (derived when the user doesn't supply explicit rules)

When the user supplies no explicit success criteria, PREPARE derives a default rule-set from the artifact's owning-skill DoD. These are starting points. Step 3 confirms or adjusts them.
...
The clean-context critic ([`../../_shared/critic.md`](../../_shared/critic.md)) was not originally designed for all artifact types. For those types PREPARE passes an F6 specialization fragment. The critic then probes the right thing. The critic's F1–F5 (vector drift, size creep, defer-vs-vector, silent edits, coverage regression) apply universally. F6 is artifact-specific:
...
Append the specialization to the critic dispatch prompt as the «F6 delta». This is the same slot `specify`/`design` use for their native F6. See [`../../_shared/critic.md`](../../_shared/critic.md) «How a skill dispatches it» step 2.
