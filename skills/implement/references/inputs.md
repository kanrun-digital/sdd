...
3. **Post-promote drift re-check (W9).** After promotion, `diff` the staged pair (`docs/features/<slug>/migrations/<NN>_*.{up,down}.sql`) against the promoted live file(s) — any **non-whitespace** difference is a finding. Catches the exact «someone hand-edited the live migration after promotion (a column type, a constraint, a default) and the design record drifted» bug, which would otherwise surface only as a silent schema/data divergence in prod. A drift finding is surfaced in the task's commit message + the tracker, and is carried forward to `review` (it's a contract-fidelity smell). Whitespace-only diffs (trailing newline, line-ending) are ignored.
4. **Commit** the promoted live file(s) with the migration task (the staged pair under `docs/features/<slug>/migrations/` was already committed by `data-model`). If the post-promote drift check (step 3) found a non-whitespace difference, the commit message names it: `migration: <slug> promote (drift: <file> — flagged for review)`.
