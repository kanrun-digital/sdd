...
dashboard_enabled: false   # Claude Code only: true → opt into the SDD visual dashboard; Codex/Cursor stop with compatibility guidance
...
- **`dashboard_enabled`** — `true | false` (default `false`). Claude Code-only opt-in for the **SDD visual dashboard**. Its `sdd-dashboard` MCP server (auto-started from `.mcp.json`) binds a loopback HTTP+WS listener and drives the live Claude session. Codex/Cursor do not load that Claude-channel integration; `start` prints a compatibility note and stops before touching Claude state. The markdown skills are unaffected. On Claude, run `/sdd:start` after enabling and install Bun. (The `implement` engine does not read it. The dashboard server + the `start` skill read it.)
