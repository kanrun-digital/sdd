...
The engine has three modes: **sequential single-agent TDD**, **agent team** (TeamCreate), and **dynamic Workflow**. Sequential is the floor. Every degrade path ends at it. The choice is deterministic. The engine makes no judgment call at runtime.
...
| `Workflow` tool not available at runtime | Skip the workflow branch. Continue with team (if eligible) or sequential. Degrade gracefully — never error. |
| `TeamCreate` not available at runtime | Skip the team branch. Continue with workflow or sequential. |
