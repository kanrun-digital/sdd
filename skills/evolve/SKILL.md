...
The **repo-level learning engine** turns the SDD pipeline's own byproducts into compact, project-specific rules that **override** the built-in skill defaults. The byproducts are `_fixes/` records from `fix`, `_review/` records from `review`, `sad.md` §9 Risks, and Accepted ADRs. Every bug the pipeline fixes is evidence. Every review finding it raises is evidence. Each shows where a skill's default was not enough for *this* repo. `evolve` aggregates that evidence into rules. The skills read those rules at startup and prioritize them over their own defaults. This uses the same precedence as nested `CLAUDE.md` files — more specific context wins.

It is **orthogonal to the backbone**: not a stage in `survey → … → ship`. It is a repo-level utility — one `docs/.skill-context/` tree serves every feature. Invoke it periodically: after 5-10 fixes, after a review cycle, or when a pattern recurs. Output is the skill-context files + an evolution log. `evolve` does not commit. The user commits the skill-context + log as a repo-level change.

Evolution-report prose follows `artifact_language`. **Skill-context rule files are always written in English.** AI agents consume them across sessions. English ensures consistent interpretation regardless of the repo's artifact language → [`../_shared/artifact-language.md`](../_shared/artifact-language.md).

## Owner

Tech Lead (they own the repo's engineering conventions). They approve which rules are written to skill-context — `evolve` proposes, the Tech Lead disposes. Consult the PM when a rule touches product-side skills (`specify`, `clarify`).
