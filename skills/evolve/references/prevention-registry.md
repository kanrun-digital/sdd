...
A flat table — one row per **independent prevention point**, NOT per evidence file. A single fix-record with 3 prevention points produces 3 rows. A prevention point targeting 2 skills appears once with both listed (and EACH skill is checked independently in step 6).
...
**Critical rules:**
- **One prevention point = one row.** Do NOT merge independent actions into a single vague summary. «Handle nulls and promises properly» is invalid — two rows.
- **Preserve concrete formats/patterns verbatim.** If the evidence specifies an exact command, syntax, assertion, or template, the prevention point includes it verbatim — not paraphrased into a vague description.
- **Target the right skill(s).** A prevention point about *writing* null-checks targets `implement`; one about *fixing* a null-relation recurrence targets `fix`; one about *validating* a schema has no nullable-without-default targets `data-model`. The same underlying lesson may target multiple skills — list all, and step 6 checks each independently.
...
4. **Follow-ups** — each independent follow-up is a candidate prevention point (but only if it's a *generalizable* lesson, not a one-off todo).

**Trap — recurrence:** if the `_fixes/` dir shows the same symptom was fixed before, that's a recurrence — the previous fix's prevention point didn't land. Extract a *strengthened* prevention point (the old one was too weak), and note the recurrence in the Source column.
...
1. **Findings with verdict `Fix`** — each is a concrete defect the reviewer caught. The *class* of defect (SQL-injection risk, missing index, N+1 query, secret logged, a11y violation) is the prevention point — target the skill that *produced* the code (usually `implement`), and sometimes the skill that *designed* it (`api` for a missing rate-limit, `sequences` for a missing compensation flow).
2. **Recurring findings across reviews** — the same finding class appearing in ≥2 review-records is a systemic skill gap (the skill keeps producing the defect). Weight these highest in gap analysis.
3. **Findings with verdict `Defer`** — each deferred finding is a known debt; if it recurs, it's a prevention point (the defer didn't resolve the underlying skill gap).
...
These are weaker evidence than fix/review-records (they're *anticipated* risks, not *observed* defects) — weight them lower in gap analysis, and only extract a prevention point when the risk/assumption names a concrete skill behavior.
...
ADRs are also the source for the **stale-rule check** (step 4): if a skill-context rule contradicts an Accepted ADR, that's a Case B conflict for the user to resolve.
