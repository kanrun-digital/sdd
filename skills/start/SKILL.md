...
`dashboard` MCP server (Bun + `Bun.serve()`), embedded in the same process that holds this
...
**writes the dashboard URL to `~/.claude/dashboard/current.url`**. So on the common path `start`
...
- `~/.claude/dashboard/current.url` — the file the server writes when it binds: line 1 is the
...
- (Optional, project-level override) `docs/.skill-context/sdd-start/SKILL.md` — if it exists, read it and treat its rules as project-level overrides (conflict → they win; apply to all outputs) → [`../_shared/skill-context.md`](../_shared/skill-context.md). Absent → no-op (defaults apply).
...
3. **(Fallback) check Bun + the MCP server.** Run `bun --version`; if Bun is missing, print «The dashboard
   ...
   `dashboard` server may have failed to boot — Bun missing, or `.mcp.json` not picked up; re-open the
   session) and **stop**.
...
- The dashboard URL printed — read from `~/.claude/dashboard/current.url` on the common path, or
